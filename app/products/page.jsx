...
function ProductsPageContent() {
  ...
  const triggerFly = useCallback((clientX, clientY, imageUrl) => {
  ...
  const handleAddToCart = useCallback((e, product) => {
    ...
    saveCart(cart);
    triggerFly(clientX, clientY, product.image_url);
  }, [triggerFly]);
    ...
    return (
      <div className="prod-card" style={{
        ...
        <div style={{ position: 'relative', background: '#f5f5f5', aspectRatio: '1/1', overflow: 'hidden' }}>
          ...
          {/* + button / stepper — bottom right of image */}
          {!outOfStock && (
            <div
            ...
            >
              {inCart ? (
                ...
                <button
                  ...
                  className="add-btn"
                  style={{ width: 34, height: 34, borderRadius: '50%', border: '1.5px solid #e0e0e0', background: '#fff', color: '#111', fontSize: 22, fontWeight: 300, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.12)', lineHeight: 1 }}>
                  +
                </button>
              )}
            </div>
  ...
  return (
    <>
      ...
      {/* Flying items — rendered in a fixed portal-like layer */}
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 9999 }}>
        {flyingItems.map(fly => (
          <div key={fly.id} style={{
            ...
            animation: 'flyToCart 0.7s cubic-bezier(0.55, 0, 1, 0.45) forwards',
