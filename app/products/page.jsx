...
function SkeletonCard() {
 return (
   <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #f0f0f0', overflow: 'hidden' }}>
     <div style={{ height: 160, background: 'linear-gradient(90deg,#f7f7f7 25%,#efefef 50%,#f7f7f7 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite' }} />
     <div style={{ padding: '12px' }}>
       ...
       <div style={{ height: 10, width: '60%', background: 'linear-gradient(90deg,#f7f7f7 25%,#efefef 50%,#f7f7f7 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite', borderRadius: 6, marginBottom: 14 }} />
       <div style={{ height: 34, background: 'linear-gradient(90deg,#f7f7f7 25%,#efefef 50%,#f7f7f7 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite', borderRadius: 8 }} />
     </div>
   </div>
 );
}
...
function ProductsPageContent() {
 ...
 const fetchProducts = useCallback(async () => {
   try {
     ...
     if (Array.isArray(data)) {
       setProducts(data);
       const firstCat = [...new Set(data.map(p => p.category).filter(Boolean))].sort()[0];
 ...
 const subcategories = useMemo(() => {
   ...
   return [...new Set(
     products.filter(p => p.category === activeCategory).map(p => p.subcategory).filter(Boolean)
   )].sort();
 }, [products, activeCategory]);

 // Products grouped by subcategory for active category
 const groupedProducts = useMemo(() => {
   let list = products.filter(p => p.category === activeCategory);
   if (activeSubcat) {
     return { [activeSubcat]: list.filter(p => p.subcategory === activeSubcat) };
   }
   ...
   if (noSubcat.length > 0) groups['অন্যান্য'] = noSubcat;
   return groups;
   ...
   return (
     <div
       ...
       <div style={{ position: 'relative', background: '#f5f5f5', aspectRatio: '1/1', overflow: 'hidden' }}>
         {p.image_url
           ? <img src={p.image_url} alt={p.name} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', transition: 'transform 0.35s' }} className="prod-img" loading="lazy" />
           : (
             <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
               <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#d0d0d0" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg>
             </div>
           )
         }
         {outOfStock && (
         ...
         {discount && !outOfStock && (
           <span style={{ position: 'absolute', top: 8, left: 8, background: '#e8f5e9', color: '#2e7d32', fontSize: 10, fontWeight: 800, padding: '3px 8px', borderRadius: 20 }}>Save {discount}%</span>
         )}
         {inCart && (
         ...
         {!outOfStock && !inCart && (
           <button
             ...
             className="plus-btn"
             style={{ position: 'absolute', bottom: 8, right: 8, width: 32, height: 32, borderRadius: '50%', background: '#fff', border: '1.5px solid #e0e0e0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 300, color: '#333', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', transition: 'all 0.15s' }}>
             +
           </button>
         )}
       </div>
       ...
       <div style={{ padding: '10px 10px 12px' }}>
         ...
         {/* Cart control */}
         {!outOfStock && inCart && (
           <div onClick={e => e.stopPropagation()} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#111', borderRadius: 8, overflow: 'hidden', height: 34 }}>
   ...
   <>
     <style>{`
       ...
       *, *::before, *::after { font-family: 'Hind Siliguri', sans-serif; box-sizing: border-box; margin: 0; padding: 0; }
       ...
       .plus-btn:hover { background: #111 !important; color: #fff !important; border-color: #111 !important; transform: scale(1.1); }
       .cat-tab { cursor: pointer; white-space: nowrap; border: none; background: transparent; font-family: 'Hind Siliguri', sans-serif; transition: all 0.15s; }
     ...
     <div style={{ minHeight: '100vh', background: '#f7f7f7', display: 'flex', flexDirection: 'column', paddingBottom: cartCount > 0 ? 80 : 0 }}>
       ...
       <div style={{ background: '#fff', borderBottom: '1px solid #ebebeb', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 1px 8px rgba(0,0,0,0.04)' }}>
           ...
           {categories.map(cat => (
             <button
               ...
               style={{
                 ...
                 borderBottom: `2.5px solid ${activeCategory === cat ? '#111' : 'transparent'}`,
               }}>
               {cat}
             </button>
           ))}
         ...
         {subcategories.length > 0 && (
           <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', overflowX: 'auto', scrollbarWidth: 'none', borderTop: '1px solid #f5f5f5' }}>
             {/* Sort icon pill */}
             <button className="subcat-chip" onClick={() => setActiveSubcat('')} style={{ flexShrink: 0, width: 34, height: 34, borderRadius: '50%', border: '1.5px solid #e0e0e0', background: activeSubcat === '' ? '#111' : '#fff', color: activeSubcat === '' ? '#fff' : '#666', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>
               ↕
             </button>
             {subcategories.map(sub => (
               <button
                 ...
                 style={{
                   ...
                   fontSize: 12,
                   fontWeight: 700,
                   border: `1.5px solid ${activeSubcat === sub ? '#111' : '#e0e0e0'}`,
                   background: activeSubcat === sub ? '#111' : '#fff',
                   color: activeSubcat === sub ? '#fff' : '#555',
                 }}>
                 {sub}
               </button>
             ))}
       ...
       {/* ── Main Content ── */}
       <div ref={mainRef} style={{ flex: 1, padding: isMobile ? '12px 10px' : '16px 20px', maxWidth: 1200, width: '100%', margin: '0 auto' }}>
         {loading ? (
           ...
           Object.entries(groupedProducts).map(([subcat, items]) => (
             <div
               ...
               ref={el => subcatRefs.current[subcat] = el}
               style={{ marginBottom: 28 }}>
               {/* Section heading */}
               <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                 <h2 style={{ fontSize: 16, fontWeight: 800, color: '#111', letterSpacing: -0.3 }}>{subcat}</h2>
